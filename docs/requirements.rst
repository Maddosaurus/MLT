Requirements
^^^^^^^^^^^^^^^^^^^^^
This framework is actively developed on Windows and Linux, OSX is tested sometimes, so it should mostly be fine.  
A Pipfile.lock is not tracked by the VCS because some stuff breaks between OSes, mostly regarding Tensorflow and CUDA.  

Most of the requirements are covered in the Pipfile, but there are some things to consider:
